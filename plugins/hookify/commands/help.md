...
- **Stop**: When Quilloom wants to stop working
- **UserPromptSubmit**: When user submits a prompt

These hooks read configuration files from `.quilloom/hookify.*.local.md` and check if any rules match the current operation.

### 2. Configuration Files

Users create rules in `.quilloom/hookify.{rule-name}.local.md` files:
...
The message body is what Quilloom sees when the rule triggers.
...
Create `.quilloom/hookify.my-rule.local.md` with the format above.
...
**Rule Files**: Keep rules in `.quilloom/hookify.*.local.md` - they should be git-ignored (add to .gitignore if needed).
...
**Hook not triggering:**
- Check rule file is in `.quilloom/` directory
...
2. Try to trigger it:
   - Ask Quilloom to run `rm -rf /tmp/test`
   - You should see the warning

4. Refine the rule by editing `.quilloom/hookify.warn-rm.local.md`
