...
```
{
  ...
  "prompt": "You are analyzing a Quilloom conversation to find behaviors the user wants to prevent.
...
2. Corrections or reversions (user fixing Quilloom's actions)
...
For each confirmed behavior, create a `.quilloom/hookify.{rule-name}.local.md` file:
...
{Message to show Quilloom when rule triggers}
```
...
**IMPORTANT**: Rule files must be created in the current working directory's `.quilloom/` folder, NOT the plugin directory.

Use the current working directory (where Quilloom was started) as the base path.

1. Check if `.quilloom/` directory exists in current working directory
   - If not, create it first with: `mkdir -p .quilloom`

2. Use Write tool to create each `.quilloom/hookify.{name}.local.md` file
   - Use relative path from current working directory: `.quilloom/hookify.{name}.local.md`
   - The path should resolve to the project's .quilloom directory, not the plugin's

3. Show user what was created:
   ```
   Created 3 hookify rules:
   - .quilloom/hookify.dangerous-rm.local.md
   - .quilloom/hookify.console-log.local.md
   - .quilloom/hookify.sensitive-files.local.md
...
4. Create `.quilloom/hookify.dangerous-rm.local.md`:
   ```markdown
   ---
...
- **File location**: Create files in project's `.quilloom/` directory (current working directory), NOT the plugin's .quilloom/
...
2. Ensure `.quilloom/` directory exists (create with mkdir if needed)
3. Use absolute path if needed: `{cwd}/.quilloom/hookify.{name}.local.md`
4. Verify file was created with Glob or ls

**If rule doesn't trigger after creation:**
1. Verify file is in project `.quilloom/` not plugin `.quilloom/`
