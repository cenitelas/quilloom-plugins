...
This analyzes your request and creates `.quilloom/hookify.warn-rm.local.md`.

### 2. Test It Immediately

**No restart needed!** Rules take effect on the very next tool use.

Ask Quilloom to run a command that should trigger the rule:
```
Run rm -rf /tmp/test
```
...
- **`stop`**: Triggers when Quilloom wants to stop (for completion checks)
...
**This rule blocks the operation** - Quilloom will not be allowed to execute these commands.
...
**This rule warns but allows** - Quilloom sees the message but can still proceed.
...
**This blocks Quilloom from stopping** if no test commands appear in the session transcript. Enable only when you want strict enforcement.
...
```bash
rm .quilloom/hookify.my-rule.local.md
```
...
This plugin is part of the Quilloom Marketplace. It should be auto-discovered when the marketplace is installed.
...
**Rule not triggering:**
1. Check rule file exists in `.quilloom/` directory (in project root, not plugin directory)
