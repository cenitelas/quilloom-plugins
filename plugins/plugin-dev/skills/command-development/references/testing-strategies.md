...
head -n 20 .quilloom/commands/my-command.md | grep -A 10 "^---"

# Check for closing frontmatter marker
head -n 20 .quilloom/commands/my-command.md | grep -c "^---" # Should be 2

# Verify file has .md extension
ls .quilloom/commands/*.md

# Check file is in correct location
test -f .quilloom/commands/my-command.md && echo "Found" || echo "Missing"
```
...
# 5. Check debug logs
tail -f ~/.quilloom/debug-logs/latest
...
cat > .quilloom/commands/test-bash.md << 'EOF'
---
...
cat > .quilloom/commands/test-forbidden.md << 'EOF'
---
...
# Command: .quilloom/commands/risky-operation.md
...
COMMANDS_CHANGED=$(git diff --cached --name-only | grep "\.quilloom/commands/.*\.md")
  ...
  test:
    ...
    steps:
      ...
      - name: Validate command structure
        run: |
          for cmd in .quilloom/commands/*.md; do
            echo "Testing: $cmd"
      ...
      - name: Check for TODOs
        run: |
          if grep -r "TODO" .quilloom/commands/; then
...
```bash
# Monitor Quilloom during command execution
# In terminal 1:
quilloom --debug

# In terminal 2:
watch -n 1 'ps aux | grep quilloom'
...
```bash
# Check file location
ls -la .quilloom/commands/my-command.md

# Check permissions
chmod 644 .quilloom/commands/my-command.md

# Check syntax
head -n 20 .quilloom/commands/my-command.md
...
```bash
# Verify syntax
grep '\$1' .quilloom/commands/my-command.md
grep '\$ARGUMENTS' .quilloom/commands/my-command.md

# Test with simple command first
echo "Test: \$1 and \$2" > .quilloom/commands/test-args.md
```
...
```bash
# Check allowed-tools
grep "allowed-tools" .quilloom/commands/my-command.md

# Verify command syntax
grep '!\`' .quilloom/commands/my-command.md
