...
Deployment state saved to `.quilloom/deployment-state.local.md`:
...
Reading deployment state from `.quilloom/deployment-state.local.md`...
...
```markdown
.quilloom/plugin-name-workflow.local.md:
...
Reading workflow state from .quilloom/plugin-name-workflow.local.md...

Current stage: @.quilloom/plugin-name-workflow.local.md
...
State file: @.quilloom/plugin-name-workflow.local.md
...
Creating: .quilloom/feature-complete.flag
...
if [ -f .quilloom/feature-complete.flag ]; then
...
if [ -f .quilloom/deployment.lock ]; then
...
!`validate.sh`
echo "checkpoint:validation" >> .quilloom/deployment-checkpoints.log

## Checkpoint 2: Build
!`build.sh`
echo "checkpoint:build" >> .quilloom/deployment-checkpoints.log

## Checkpoint 3: Deploy
!`deploy.sh`
echo "checkpoint:deploy" >> .quilloom/deployment-checkpoints.log
...
Written to .quilloom/deployment-state.local.md
...
Reading state: @.quilloom/deployment-state.local.md
...
rm .quilloom/deployment-state.local.md
