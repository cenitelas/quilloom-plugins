...
if [ ! -f ".quilloom/command-initialized" ]; then
...
Checking for user config: .quilloom/plugin-name.local.md

if [ -f ".quilloom/plugin-name.local.md" ]; then
  # Parse YAML frontmatter for settings
  VERBOSE=$(grep "^verbose:" .quilloom/plugin-name.local.md | cut -d: -f2 | tr -d ' ')
  COLOR=$(grep "^color:" .quilloom/plugin-name.local.md | cut -d: -f2 | tr -d ' ')
  MAX_RESULTS=$(grep "^max_results:" .quilloom/plugin-name.local.md | cut -d: -f2 | tr -d ' ')

  echo "✓ Using user configuration"
else
  echo "Using default configuration"
  echo "Create .quilloom/plugin-name.local.md to customize"
fi
...
Or set in .quilloom/plugin-name.local.md:
\`\`\`yaml
---
format: yaml
output: custom.txt
...
if [ -f ".quilloom/operation-completed.flag" ]; then
  ℹ️  Operation already completed

  Completed at: $(cat .quilloom/operation-completed.flag)

  To re-run:
  1. Remove flag: rm .quilloom/operation-completed.flag
  2. Run command again
...
Marking complete...
echo "$(date)" > .quilloom/operation-completed.flag
```
