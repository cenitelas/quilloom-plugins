...
Detailed analysis of how production plugins use the `.quilloom/plugin-name.local.md` pattern.
...
SWARM_STATE_FILE=".quilloom/multi-agent-swarm.local.md"
...
```bash
cat > "$WORKTREE_PATH/.quilloom/multi-agent-swarm.local.md" <<EOF
---
...
sed "s/^pr_number: .*/pr_number: $PR_NUM/" \
  ".quilloom/multi-agent-swarm.local.md" > temp.md
mv temp.md ".quilloom/multi-agent-swarm.local.md"
```
...
**Purpose:** Prevent session exit and loop Quilloom's output back as input
...
RALPH_STATE_FILE=".quilloom/ralph-loop.local.md"
...
# Create state file
cat > ".quilloom/ralph-loop.local.md" <<EOF
---
iteration: 1
...
echo "Ralph loop initialized: .quilloom/ralph-loop.local.md"
```
...
|---------|-------------------|--------------|
| **File** | `.quilloom/multi-agent-swarm.local.md` | `.quilloom/ralph-loop.local.md` |
...
```bash
# BAD
FILE="/Users/alice/.quilloom/my-plugin.local.md"

# GOOD
FILE=".quilloom/my-plugin.local.md"
```
...
The `.quilloom/plugin-name.local.md` pattern provides:
