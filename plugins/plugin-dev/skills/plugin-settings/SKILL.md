...
description: This skill should be used when the user asks about "plugin settings", "store plugin configuration", "user-configurable plugin", ".local.md files", "plugin state files", "read YAML frontmatter", "per-project plugin settings", or wants to make plugin behavior configurable. Documents the .quilloom/plugin-name.local.md pattern for storing plugin-specific configuration with YAML frontmatter and markdown content.
version: 0.1.0
---

# Plugin Settings Pattern for Quilloom Plugins

## Overview

Plugins can store user-configurable settings and state in `.quilloom/plugin-name.local.md` files within the project directory. This pattern uses YAML frontmatter for structured configuration and markdown content for prompts or additional context.

**Key characteristics:**
- File location: `.quilloom/plugin-name.local.md` in project root
...
- Prompts to feed back to Quilloom
- Documentation or notes
```

### Example: Plugin State File

**.quilloom/my-plugin.local.md:**
```markdown
---
enabled: true
strict_mode: false
...
STATE_FILE=".quilloom/my-plugin.local.md"
...
Steps:
1. Check if settings exist at `.quilloom/my-plugin.local.md`
...
Check for plugin settings at `.quilloom/my-plugin.local.md`.
...
```bash
#!/bin/bash
STATE_FILE=".quilloom/security-scan.local.md"
...
**.quilloom/multi-agent-swarm.local.md:**
```markdown
---
...
2. Create `.quilloom/my-plugin.local.md` with YAML frontmatter
3. Set appropriate values based on user input
4. Inform user that settings are saved
5. Remind user to restart Quilloom for hooks to recognize changes
```
...
Create `.quilloom/my-plugin.local.md` in your project:
...
After creating or editing, restart Quilloom for changes to take effect.
```
...
✅ **DO:**
- Use `.quilloom/plugin-name.local.md` format
- Match plugin name exactly
- Use `.local.md` suffix for user-local files

❌ **DON'T:**
- Use different directory (not `.quilloom/`)
...
**Important:** Settings changes require Quilloom restart.
...
After editing `.quilloom/my-plugin.local.md`:
1. Save the file
2. Exit Quilloom
...
```bash
if [[ ! -f ".quilloom/my-plugin.local.md" ]]; then
...
3. Add gitignore entry for `.quilloom/*.local.md`
...
7. Remind users that changes require Quilloom restart
