...
Complete guide to using MCP tools effectively in Quilloom plugin commands and agents.

## Overview

Once an MCP server is configured, its tools become available with the prefix `mcp__plugin_<plugin-name>_<server-name>__<tool-name>`. Use these tools in commands and agents just like built-in Quilloom tools.
...
- Can use any tool Quilloom determines is necessary
...
Quilloom automatically structures tool calls based on schema:

```typescript
// Quilloom generates this internally
{
...
```markdown
Steps:
1. Make parallel calls (Quilloom handles this automatically):
...
2. **Install plugin locally** in `.quilloom-plugin/`
3. **Verify tools available** with `/mcp`
4. **Test command** that uses tools
5. **Check debug output**: `quilloom --debug`
