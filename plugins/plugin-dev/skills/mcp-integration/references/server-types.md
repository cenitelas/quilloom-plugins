...
Complete reference for all MCP server types supported in Quilloom plugins.
...
1. **Startup**: Quilloom spawns process with `command` and `args`
2. **Communication**: JSON-RPC messages via stdin/stdout
3. **Lifecycle**: Process runs for entire Quilloom session
4. **Shutdown**: Process terminated when Quilloom exits
