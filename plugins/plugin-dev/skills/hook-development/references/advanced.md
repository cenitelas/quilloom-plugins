...
if [ -f ".quilloom-hooks-config.json" ]; then
  strict_mode=$(jq -r '.strict_mode' .quilloom-hooks-config.json)
...
**Example .quilloom-hooks-config.json:**
```json
{
  "strict_mode": true,
...
# Append to audit log
echo "$timestamp | $USER | $tool_name | $input" >> ~/.quilloom/audit.log
