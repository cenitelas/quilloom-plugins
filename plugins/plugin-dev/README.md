...
A comprehensive toolkit for developing Quilloom plugins with expert guidance on hooks, MCP integration, plugin structure, and marketplace publishing.

## Overview

The plugin-dev toolkit provides seven specialized skills to help you build high-quality Quilloom plugins:
...
4. **Plugin Settings** - Configuration patterns using .quilloom/plugin-name.local.md files
...
7. **Testing** - Verify plugin works in Quilloom
...
**What it covers:**
- .quilloom/plugin-name.local.md pattern for configuration
...
- AI-assisted agent generation using Quilloom's proven prompt
...
- 3 reference docs: agent-creation-system-prompt (from Quilloom), system-prompt-design (~4,000w), triggering-examples (~2,500w)
...
- Based on skill-creator methodology adapted for Quilloom plugins
...
Install from quilloom-marketplace:

```bash
/plugin install plugin-dev@quilloom-marketplace
```
...
This keeps Quilloom's context focused while providing deep knowledge when needed.
...
- Based on official Quilloom documentation
...
This plugin is part of the quilloom-marketplace. To contribute improvements:
